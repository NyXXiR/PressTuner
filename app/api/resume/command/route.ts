...
import { planResumeAiCommand } from "@/lib/services/resume/resumeAiOrchestrator";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const plan = await planResumeAiCommand({
    ...
    void trackOpsEvent({
      ...
      properties: {
        scope: "resume:command",
        route: "/api/resume/command",
        errorCode: error?.code ?? "RESUME_COMMAND_FAILED",
    ...
    const err = apiError(
      error?.code ?? "RESUME_COMMAND_FAILED",
      error?.message ?? "Failed to process resume command",
      status,
