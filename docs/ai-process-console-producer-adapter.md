...
Completed v2 results may add the safe optional receipt `providerPublication: { langsmith, posthog }`. Each value is exactly `NOT_CONFIGURED`, `NOT_ATTEMPTED`, or `ATTEMPTED`. Missing configuration is reported before provider work; bounded local metadata/time skips are not attempted; and the state changes to attempted immediately when a client or fetch operation is invoked, even if it throws, aborts, times out, or returns an error. The receipt never carries an identifier, URL, payload, credential, timestamp, count, or error. Exact terminal command replay republishes with the same deterministic LangSmith run ID and PostHog `$insert_id` and returns a fresh receipt. Rejected, malformed, nonterminal, or unexpectedly failed publisher paths omit the receipt instead of inventing certainty.
