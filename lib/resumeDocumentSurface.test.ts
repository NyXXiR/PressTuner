...
test("resume documents exchange scoped AI edits as one validated JSON bundle", async () => {
  ...
  assert.match(dialog, /reviewResumeAiEdit/);
  ...
  assert.match(dialog, /resumeAiEditTargetOptions/);
  assert.match(dialog, /prepareTargetedResumeAiEdit/);
  assert.match(dialog, /선택한 섹션/);
  assert.match(dialog, /전체 승인·반영/);
  assert.match(dialog, /data-ai-review-actions/);
  assert.match(dialog, /shadow-\[0_-8px_24px/);
  ...
  assert.match(contract, /baseSectionFingerprints/);
  assert.match(contract, /editableSectionIds/);
  assert.match(contract, /RESUME_AI_EDIT_SECTION_OUT_OF_SCOPE/);
  assert.match(contract, /RESUME_AI_EDIT_CONTEXT_CHANGED/);
  assert.match(contract, /acceptedOperations/);
  assert.match(contract, /RESUME_AI_EDIT_OPERATION_NO_CHANGE/);
  assert.match(contract, /RESUME_AI_EDIT_SECTION_CHANGED/);
...
test("resume documents expose canonical career details with type and relationship controls", async () => {
  ...
  assert.match(builder, /alignedModalField/);
  assert.match(builder, /grid-rows-\[auto_2\.5rem_minmax\(2\.5rem,auto\)\]/);
  assert.match(builder, /연결 확인 필요/);
  assert.match(panel, /payload\.detailLabel \?\? ""/);
  assert.match(panel, /detailLabel: event\.target\.value/);
  assert.match(panel, /alignedImportField/);
