...
export async function retryDebugAttempt(args: { teamId: string; userId: string; attemptId: string; input: z.infer<typeof RetryDebugAttemptSchema> }) {
  ...
  const child = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.pressAiDebugAttempt.create({ data: { id: args.input.commandId, teamId: args.teamId, createdById: args.userId, caseId: parent.caseId, caseRevision: parent.caseRevision, topologySnapshot: json(parent.topologySnapshot), guardrailSnapshot: json(parent.guardrailSnapshot), captureInputSnapshot: parent.captureInputSnapshot === null ? undefined : json(rebase(parent.captureInputSnapshot, parent.articleId, article.id)), currentIteration: parent.currentIteration, agentRunId: run.id, parentAttemptId: parent.id, baselineAttemptId: parent.baselineAttemptId ?? parent.id, processId: parent.processId, processVersion: parent.processVersion, registryHash: parent.registryHash, executorVersion: parent.executorVersion, startNodeId: retryNode.id, activeNodeId: retryNode.id, status: "ACTIVE", articleId: article.id, inputSnapshot: json(rebasedInput) } });
    for (const checkpoint of parent.checkpoints.filter((item) => item.sequence < retryNode.sequence)) await tx.pressAiDebugCheckpoint.create({ data: { attemptId: created.id, nodeId: checkpoint.nodeId, sequence: checkpoint.sequence, iteration: checkpoint.iteration, mode: "RESTORED", input: json(rebase(checkpoint.input, parent.articleId, article.id)), output: json(rebase(checkpoint.output, parent.articleId, article.id)), restoredFromCheckpointId: checkpoint.id, quotaUnits: 0, processVersion: checkpoint.processVersion, registryHash: checkpoint.registryHash, executorVersion: checkpoint.executorVersion } });
