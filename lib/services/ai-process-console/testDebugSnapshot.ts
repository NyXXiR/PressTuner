...
export async function captureTestDebugSnapshot(receiptId: string, attemptId: string): Promise<StoredTestDebugSnapshot> {
  const attempt = await prisma.pressAiDebugAttempt.findUniqueOrThrow({
    ...
    select: {
      ...
      transitions: { orderBy: { sequence: "asc" }, select: { edgeId: true, sourceNodeId: true, targetNodeId: true, targetPayload: true, verdict: true, sourceCheckpoint: { select: { input: true, output: true } }, observations: { orderBy: [{ displayOrder: "asc" }, { guardrailId: "asc" }], select: { guardrailId: true, verdict: true, origin: true } } } },
  ...
  const snapshot = StoredTestDebugSnapshotSchema.parse({
    ...
    transitions: attempt.transitions.map((transition) => ({
      transitionId: transition.edgeId, sourceNodeId: transition.sourceNodeId, targetNodeId: transition.targetNodeId,
      ...
      decision: { decisionRef: `presstuner:decision:${transition.edgeId}:3.0.0`, matched: transition.verdict !== "BLOCK" },
