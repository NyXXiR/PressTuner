...
test("v2 fixtures preserve existing order and append exact transition scenarios", () => {
  assert.deepEqual(fixtureRegistryV2.map(({ fixture }) => fixture.fixtureId), ["success-v2", "final-quality-block-v2", "brief-draft-warn-v2", "brief-draft-block-v2", "success-v2-3-1"]);
  assert.deepEqual(fixtureRegistryV2.map(({ fixture }) => fixture.scenario), ["SUCCESS", "QUALITY_BLOCK", "TRANSITION_WARN", "TRANSITION_BLOCK", "SUCCESS"]);
