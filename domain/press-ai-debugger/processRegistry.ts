...
export type PressAiProcessEdge = Readonly<{
  ...
  userGateInputFields?: readonly string[];
  kind?: "FORWARD" | "ITERATION";
}>;
...
export const pressCreationProcess = Object.freeze({
  id: "press-creation", version: "3.0.0", label: "보도자료 작성", description: "체크포인트를 실행하고 전이를 별도로 검사하는 보도자료 작성 흐름입니다.",
  nodes: Object.freeze([
    pressNode({ id: "article-initialization", sequence: 0, label: "문서 초기화", operation: "initArticleDraft", operationKey: "article.initialize", description: "새 PRESS_RELEASE 테스트 문서를 확인합니다.", troubleshooting: "문서 ID와 팀 소유권을 확인하세요.", inputSchema: z.union([z.object({ articleId: z.string().min(1) }), z.object({ type: z.literal("PRESS_RELEASE") })]), outputSchema: z.object({ articleId: z.string().min(1), teamId: z.string().optional(), type: z.literal("PRESS_RELEASE").optional() }), contract: { requiredInputFields: ["articleId"], optionalInputFields: [], outputFields: ["articleId", "teamId", "type"], requires: [], provides: ["article.initialized"], allowIncoming: false }, metricIds: [], findingIds: [], client: { stepId: "init", method: "POST", path: "/api/articles/init", hasBody: true } }),
    pressNode({ id: "brief-normalization", sequence: 1, label: "메모 정규화", operation: "normalizeBriefUseCase", operationKey: "press.normalize-brief", description: "메모를 편집 가능한 브리프로 정리합니다.", troubleshooting: "사실, 날짜, 인용과 제한 조건을 확인하세요.", inputSchema: z.object({ articleId: z.string(), rawText: z.string().min(1), tone: z.enum(["formal", "neutral", "friendly"]) }), outputSchema: pressBriefSchema, contract: { requiredInputFields: ["articleId", "rawText", "tone"], optionalInputFields: [], outputFields: ["articleId", "serviceName", "announceType", "oneLiner", "points", "quoteMessage", "quoteWho", "tone", "rawText", "eventAt", "publishAt"], requires: ["article.initialized"], provides: ["brief.normalized"], allowIncoming: true }, gate: { id: "confirm-normalized-brief", label: "정규화 브리프 확인" }, metricIds: ["factCandidates"], findingIds: ["quality-warning"], quotaUnits: getAiQuotaActionDefinition("press_brief_normalize").units, client: { stepId: "normalize", method: "POST", path: "/api/articles/{articleId}/brief/normalize", hasBody: true, needsArticle: true } }),
    pressNode({ id: "draft-generation", sequence: 2, label: "초안 생성", operation: "generateArticleFromBrief", operationKey: "press.generate-draft", description: "확인한 브리프로 보도자료 초안을 만듭니다.", troubleshooting: "제목과 본문의 사실·주의 문구를 확인하세요.", inputSchema: pressBriefSchema.extend({ articleId: z.string() }), outputSchema: z.object({ articleId: z.string(), title: z.string(), plain: z.string().optional(), lead: z.string().optional(), fact: z.string().optional(), paragraphs: z.array(record).optional(), closing: z.string().optional() }).passthrough(), contract: { requiredInputFields: ["articleId", "announceType", "points", "tone"], optionalInputFields: ["serviceName", "oneLiner", "quoteMessage", "quoteWho", "rawText", "eventAt", "publishAt"], outputFields: ["articleId", "title", "plain", "lead", "fact", "paragraphs", "closing"], requires: ["brief.normalized"], provides: ["draft.reviewable"], allowIncoming: true }, gate: { id: "confirm-generated-draft", label: "생성 초안 확인" }, metricIds: [], findingIds: ["quality-warning"], quotaUnits: getAiQuotaActionDefinition("press_draft_generate").units, client: { stepId: "generate", method: "POST", path: "/api/articles/{articleId}/generate", hasBody: true, needsArticle: true } }),
    pressNode({ id: "draft-review", sequence: 3, label: "초안 리뷰", operation: "reviewUseCase", operationKey: "press.review-draft", description: "수정 제안과 위치를 만듭니다.", troubleshooting: "선택 가능한 노트 ID와 지침을 확인하세요.", inputSchema: z.object({ articleId: z.string(), title: z.string(), plain: z.string(), userInstruction: z.string().max(1000).optional() }), outputSchema: z.object({ notes: z.array(z.object({ id: z.string() }).passthrough()), spans: z.array(record).optional() }).passthrough(), contract: { requiredInputFields: ["articleId", "title", "plain"], optionalInputFields: ["userInstruction"], outputFields: ["notes", "spans"], requires: ["draft.reviewable"], provides: ["review.notes"], allowIncoming: true }, gate: { id: "select-review-notes", label: "리뷰 노트 선택" }, metricIds: ["availableNotes"], findingIds: ["no-actionable-notes"], quotaUnits: getAiQuotaActionDefinition("press_review").units, client: { stepId: "polish", method: "POST", path: "/api/articles/{articleId}/polish", hasBody: true, needsArticle: true } }),
    pressNode({ id: "selected-rewrite", sequence: 4, label: "선택 수정", operation: "rePolishUseCase", operationKey: "press.rewrite-selected", description: "선택한 리뷰 노트만 반영합니다.", troubleshooting: "선택 노트와 최종 본문을 확인하세요.", inputSchema: z.object({ articleId: z.string(), selectedNoteIds: z.array(z.string()).min(1), userInstruction: z.string().max(1000) }), outputSchema: z.object({ title: z.string(), plain: z.string() }).passthrough(), contract: { requiredInputFields: ["articleId", "selectedNoteIds", "userInstruction"], optionalInputFields: [], outputFields: ["title", "plain"], requires: ["review.notes"], provides: ["draft.reviewable"], allowIncoming: true }, metricIds: [], findingIds: ["quality-warning"], quotaUnits: getAiQuotaActionDefinition("press_rewrite").units, client: { stepId: "repolish", method: "POST", path: "/api/articles/{articleId}/re-polish", hasBody: true, needsArticle: true } }),
  ]),
  edges: Object.freeze([
    { id: "initialization-brief", sequence: 0, source: "article-initialization", target: "brief-normalization", payload: ["articleId", "rawText", "tone"], mandatoryGuardrailIds: ["article-team-ownership", "fresh-press-release"], kind: "FORWARD" },
    { id: "brief-draft", sequence: 1, source: "brief-normalization", target: "draft-generation", payload: ["articleId", "announceType", "points", "tone"], mandatoryGuardrailIds: ["memo-brief-grounding", "critical-fact-preservation"], humanGate: { id: "confirm-normalized-brief", label: "정규화 브리프 확인" }, kind: "FORWARD" },
    { id: "draft-review", sequence: 2, source: "draft-generation", target: "draft-review", payload: ["articleId", "title", "plain", "userInstruction"], mandatoryGuardrailIds: ["brief-draft-grounding", "press-structure"], userGateInputFields: ["userInstruction"], humanGate: { id: "confirm-generated-draft", label: "생성 초안 확인" }, kind: "FORWARD" },
    { id: "review-rewrite", sequence: 3, source: "draft-review", target: "selected-rewrite", payload: ["articleId", "selectedNoteIds", "userInstruction"], mandatoryGuardrailIds: ["review-note-selection", "rewrite-instruction-bounds", "review-checkpoint-lineage"], userGateInputFields: ["selectedNoteIds", "userInstruction"], humanGate: { id: "select-review-notes", label: "리뷰 노트 선택" }, kind: "FORWARD" },
    { id: "rewrite-review", sequence: 4, source: "selected-rewrite", target: "draft-review", payload: ["articleId", "title", "plain", "userInstruction"], mandatoryGuardrailIds: ["rewrite-review-lineage", "press-structure"], userGateInputFields: ["userInstruction"], humanGate: { id: "review-rewritten-draft", label: "수정본 다시 리뷰" }, kind: "ITERATION" },
  ]),
} satisfies PressAiProcessDefinition);
