...
export type ArticleItem = {
  ...
  type: "PRESS_RELEASE" | "BLOG_POST" | "ANNOUNCEMENT" | "OTHER";
  teamId: string | null;
  updatedAt: string;
  createdAt: string;
};
...
type QueryState = {
  q: string;
  status: Array<ArticleItem["status"]>;
  type: Array<ArticleItem["type"]>;
  page: number;
  pageSize: number;
};
...
type Config = {
  apiPath: string;
  defaultStatus?: Array<ArticleItem["status"]>;
  defaultType?: Array<ArticleItem["type"]>;
  mutationPath?: string;
};

export function createArticlesStore({
  apiPath,
  defaultStatus = [],
  defaultType = [],
  mutationPath,
}: Config) {
  const mutatePath = mutationPath ?? apiPath;
  const useStore = create<ArticlesStore>()((set, get) => ({
    query: {
      q: "",
    ...
    fetchList: async () => {
      ...
      if (query.type.length > 0) params.set("type", query.type.join(","));

      set((s) => ({ list: { ...s.list, loading: true, error: null } }));

      try {
        const res = await fetchWithLoading(`${apiPath}?${params.toString()}`, {
          credentials: "include",
        });
    ...
    deleteOne: async (id: string) => {
      try {
        const res = await fetchWithLoading(`${mutatePath}/${id}`, {
          method: "DELETE",
        });
    ...
    updateTeam: async (id: string, teamId: string | null) => {
      const prevItems = get().list.items;

      set((s) => ({
        list: {
          ...s.list,
          items: s.list.items.map((it) =>
            it.id === id ? { ...it, teamId } : it,
          ),
        },
      }));

      try {
        const res = await fetchWithLoading(`${mutatePath}/${id}/team`, {
          method: "PATCH",
    ...
    bulkDeleteSelected: async () => {
      ...
      try {
        const res = await fetchWithLoading(`${mutatePath}/bulk-delete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
