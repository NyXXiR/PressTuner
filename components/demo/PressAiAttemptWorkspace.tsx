...
export function PressAiAttemptWorkspace(props: {
  ...
  return (
    <div className="mt-4 min-w-0">
      <section
        className="mb-4 rounded-xl border border-primary/35 bg-primary/5 p-4"
        aria-labelledby="press-ai-persisted-attempt-heading"
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 id="press-ai-persisted-attempt-heading" className="font-black">
              저장된 시도 · 완료 체크포인트 보존
            </h2>
        ...
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-3">
          ...
          <div>
            ...
            <dd title={attempt.status}>{attemptStatusLabel(attempt.status)}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd>
              {startNode?.label ?? attempt.startNodeId} ({attempt.startNodeId})
            </dd>
          </div>
          <div>
          ...
          <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end">
            <label className="min-w-0 flex-1 text-sm font-bold">
              ...
              <select
                ...
                className="mt-1 min-h-11 w-full rounded border bg-background px-3 disabled:opacity-50"
              >
            ...
            <button
              ...
              className="min-h-11 rounded-lg border border-primary bg-background px-4 text-sm font-black text-primary disabled:opacity-50"
            >
              이 단계부터 분기
            </button>
          </div>
        ...
        {checkpointChoices.length === 0 ? (
          <p className="mt-2 text-xs text-muted-foreground">
            아직 저장된 체크포인트가 없어 단계 분기는 할 수 없습니다. 처음부터
            다시 실행할 수는 있습니다.
          </p>
        ) : null}
      </section>
