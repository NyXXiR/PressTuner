...
function SectionBody({ section, content, layout, relatedWorkItems = [] }: { section: ResumeSection; content: SectionContent; layout: SectionLayout; relatedWorkItems?: ItemContent[] }) {
  ...
  if (section.id === "projects") {
    ...
    const detailArticle = (item: ItemContent) => <article className="resume-item grid grid-cols-[26mm_1fr] gap-4" key={item.id}><p className="resume-item-period text-[9px] font-bold text-slate-500">{formatItemPeriod(item)}</p><div><p className="text-[8px] font-extrabold text-orange-600">{detailTypeLabels[item.detailType ?? "project"]}</p><h3 className="resume-item-title text-[11px] font-black">{item.title}</h3><p className="resume-item-subtitle text-[9px] font-bold text-orange-600">{item.subtitle}</p><p className="resume-item-body mt-1 whitespace-pre-line text-[9px] leading-5 text-slate-600">{item.body}</p></div></article>;
    return <div className="resume-career-detail-groups grid gap-5">{grouped.employmentGroups.filter((group) => group.details.length).map((group) => <section className="border-l-2 border-slate-300 pl-3" key={group.work.id}><h3 className="text-[11px] font-black">{group.work.title}</h3><p className="text-[9px] font-bold text-slate-500">{group.work.subtitle} · {formatItemPeriod(group.work)}</p><div className="mt-3 grid gap-3">{group.details.map(detailArticle)}</div></section>)}{grouped.independentDetails.length > 0 && <section><h3 className="mb-3 text-[10px] font-black text-slate-600">독립 프로젝트</h3><div className="grid gap-4">{grouped.independentDetails.map(detailArticle)}</div></section>}{grouped.unresolvedDetails.length > 0 && <section><h3 className="mb-3 text-[10px] font-black text-amber-700">연결 확인 필요</h3><div className="grid gap-4">{grouped.unresolvedDetails.map((item) => <div key={item.id}><p className="mb-1 text-[8px] font-bold text-amber-700">{item.relatedWorkTitle}</p>{detailArticle(item)}</div>)}</div></section>}</div>;
  }
  const items = isCareerTimelineSectionId(section.id)
    ? sortExperienceItems(itemContent.items, itemContent.sortDirection)
    : itemContent.items;
  return <div className={cx("resume-items grid", layout === "cards" ? "grid-cols-2 gap-3" : layout === "compact" ? "gap-2" : "gap-4")}>{items.map((item) => <article className={cx("resume-item", layout === "cards" && "border border-slate-200 p-3", layout === "standard" && "grid grid-cols-[26mm_1fr] gap-4")} key={item.id}><p className="resume-item-period text-[9px] font-bold text-slate-500">{formatItemPeriod(item)}</p><div><h3 className="resume-item-title text-[11px] font-black">{item.title}</h3><p className="resume-item-subtitle text-[9px] font-bold text-orange-600">{[item.relatedWorkTitle, item.subtitle].filter(Boolean).join(" · ")}</p><p className="resume-item-body mt-1 whitespace-pre-line text-[9px] leading-5 text-slate-600">{item.body}</p></div></article>)}</div>;
}
