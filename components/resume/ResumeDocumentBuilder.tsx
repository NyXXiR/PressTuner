...
function StructuredEditor({ section, content, onChange }: { section: ResumeSection; content: SectionContent; onChange: (content: SectionContent) => void }) {
  ...
  const durationControls = section.id === "experience" && <fieldset className="mb-4 border border-border bg-muted/20 p-4"><legend className="px-2 text-xs font-extrabold">경력 표시 설정</legend><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-xs font-bold text-muted-foreground">시작 연월 정렬<select className="h-10 border border-border bg-background px-3 text-sm font-normal text-foreground" value={value.sortDirection ?? ""} onChange={(event) => onChange({ ...value, sortDirection: (event.target.value || undefined) as ItemsContent["sortDirection"] })}><option value="">수동 순서</option><option value="latest-first">최신순</option><option value="oldest-first">오래된순</option></select></label><fieldset><legend className="mb-2 text-xs font-bold text-muted-foreground">총 경력</legend><div className="flex flex-wrap gap-3"><label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={!hasManualDuration} name="career-duration-mode" type="radio" onChange={() => onChange({ ...value, careerDurationOverrideMonths: undefined })} /> 자동 계산</label><label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={hasManualDuration} name="career-duration-mode" type="radio" onChange={() => onChange({ ...value, careerDurationOverrideMonths: calculateAutomaticCareerDurationMonths(value.items, currentLocalMonth()) })} /> 직접 입력</label></div>{hasManualDuration && <div className="mt-3 grid grid-cols-2 gap-2"><label className="grid gap-1 text-[11px] font-bold text-muted-foreground">경력 연<input aria-label="경력 연" className="h-10 border border-border bg-background px-3 text-sm text-foreground" min={0} type="number" value={Math.floor(durationMonths / 12)} onChange={(event) => onChange({ ...value, careerDurationOverrideMonths: normalizeCareerDurationOverride(Number(event.target.value), durationMonths % 12) })} /></label><label className="grid gap-1 text-[11px] font-bold text-muted-foreground">경력 개월<input aria-label="경력 개월" className="h-10 border border-border bg-background px-3 text-sm text-foreground" max={11} min={0} type="number" value={durationMonths % 12} onChange={(event) => onChange({ ...value, careerDurationOverrideMonths: normalizeCareerDurationOverride(Math.floor(durationMonths / 12), Number(event.target.value)) })} /></label></div>}</fieldset></div></fieldset>;
  return <div>{durationControls}<div className="grid gap-3">{value.items.map((item, index) => { const endMonthEnabled = isItemEndMonthEnabled(item) && !item.isCurrent; return <fieldset className="border border-border bg-muted/20 p-4" key={item.id}><legend className="px-2 text-xs font-extrabold">항목 {index + 1}</legend><div className="grid gap-3 sm:grid-cols-2"><MonthField label="시작 연월" value={item.startMonth} onChange={(startMonth) => updateItem(item.id, { startMonth })} /><div className="grid gap-2"><MonthField label="종료 연월" value={item.endMonth} disabled={!endMonthEnabled} onChange={(endMonth) => updateItem(item.id, { endMonth, endMonthEnabled: true })} /><label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={endMonthEnabled} type="checkbox" onChange={(event) => updateItem(item.id, { endMonthEnabled: event.target.checked, endMonth: "", ...(section.id === "experience" && event.target.checked ? { isCurrent: false } : {}) })} /> 종료연월 있음</label></div>{section.id === "experience" && <label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={Boolean(item.isCurrent)} type="checkbox" onChange={(event) => updateItem(item.id, { isCurrent: event.target.checked, endMonth: event.target.checked ? "" : item.endMonth, endMonthEnabled: event.target.checked ? false : item.endMonthEnabled })} /> 재직 중</label>}<Field label="제목" value={item.title} onChange={(title) => updateItem(item.id, { title })} /><Field label="조직·부제" value={item.subtitle} onChange={(subtitle) => updateItem(item.id, { subtitle })} /><div className="sm:col-span-2"><TextArea label="설명" value={item.body} onChange={(body) => updateItem(item.id, { body })} /></div></div><button className="mt-3 inline-flex h-9 items-center gap-2 border border-red-200 px-3 text-xs font-bold text-red-600" onClick={() => onChange({ ...value, items: value.items.filter((entry) => entry.id !== item.id) })} type="button"><Trash2 className="h-3.5 w-3.5" /> 이 항목 삭제</button></fieldset>; })}</div><button className="mt-3 inline-flex h-10 items-center gap-2 border border-primary px-4 text-sm font-bold text-primary" onClick={() => onChange({ ...value, items: [...value.items, { id: `item-${Date.now()}`, meta: "", startMonth: "", endMonth: "", endMonthEnabled: false, isCurrent: false, title: "", subtitle: "", body: "" }] })} type="button"><Plus className="h-4 w-4" /> 항목 추가</button></div>;
}
