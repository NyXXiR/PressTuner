...
export function ResumeDocumentBuilder() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const a4PageHeight = paper.clientWidth * (297 / 210);
      setPageCount(Math.max(1, Math.ceil(paper.scrollHeight / Math.max(1, a4PageHeight))));
    };
    update();
    ...
    <div className="resume-document-builder wongoji-sharp mx-auto w-full max-w-[1280px] pb-20">
      ...
      {view === "shared" ? <SharedManager profiles={state.roleProfiles} sections={state.sharedSections} onAdd={() => { setNewSectionTitle(""); setNewSectionKind("items"); setSharedSectionDialogOpen(true); }} onDelete={(section) => window.confirm(`‘${section.title}’ 공통 섹션을 모든 직군과 지원 버전에서 삭제할까요?`) && setState((current) => deleteSharedSection(current, section.id))} onEdit={(section) => openEditor("shared", section, section.content)} onLinkExperience={() => setExperienceDialogOpen(true)} onOrder={(sectionOrder) => setState((current) => updateSharedSectionOrder(current, sectionOrder))} /> : (
        <div className="resume-builder-layout mt-6 grid items-start gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="min-w-0"><div className="resume-builder-chrome mb-3 flex flex-wrap items-center justify-between gap-2 border border-primary/25 bg-primary/5 px-4 py-3 text-xs"><span className="font-bold text-primary">A4 페이지 경계가 점선으로 표시됩니다.</span><span className="hidden font-extrabold md:inline">예상 {pageCount}페이지</span><span className="font-bold text-muted-foreground md:hidden">페이지 수는 데스크톱에서 확인</span></div><article className="resume-paper resume-page-guides mx-auto w-full max-w-[210mm] bg-white text-slate-950 shadow-xl" ref={paperRef}><div className="min-h-[297mm] px-[14mm] py-[16mm]"><div className="resume-print-target mb-8 flex items-end justify-between border-b-2 border-slate-950 pb-3"><div><p className="text-[9px] font-bold tracking-widest text-slate-500">{active?.company || activeProfile.name}</p><p className="mt-1 text-sm font-black">{resolveDocumentRole(activeProfile, active)}</p></div><p className="text-xs font-bold text-slate-500">{active?.name || `${activeProfile.name} 이력서`}</p></div><p className="resume-reorder-help mb-4 flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><GripVertical className="h-3.5 w-3.5" /> 핸들을 끌어 현재 단계의 섹션 순서를 바꿀 수 있습니다.</p><Reorder.Group axis="y" className="resume-print-sections grid gap-7" onReorder={(sectionOrder) => setState((current) => active ? updateSectionOrder(current, active.id, sectionOrder) : updateRoleProfileSectionOrder(current, activeProfile.id, sectionOrder))} values={orderedSections.map((section) => section.id)}>{orderedSections.map((section) => {
...
function SectionBody({ section, content, layout }: { section: ResumeSection; content: SectionContent; layout: SectionLayout }) {
  if (section.kind === "identity") {
    ...
    return <div className={cx("grid items-end gap-6", value.photo ? "grid-cols-[minmax(0,1fr)_24mm]" : "grid-cols-1", layout === "cards" && "border border-slate-200 p-4")} data-photo-position="right"><div className="min-w-0"><div className={cx("grid items-end gap-4", layout === "compact" ? "grid-cols-1" : "grid-cols-[1fr_auto]")}><h2 className="text-3xl font-black tracking-[-.05em]">{value.name}</h2>{contactItems.length > 0 && <div className={cx("text-[9px] leading-5 text-slate-500", layout !== "compact" && "text-right")}>{contactItems.map((item) => <p key={item}>{item}</p>)}</div>}</div>{factItems.length > 0 && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-slate-200 pt-2 text-[8px] text-slate-500">{factItems.map((item) => <span key={item}>{item}</span>)}</div>}</div>{value.photo && <Image alt={`${value.name || "지원자"} 증명사진`} className="h-[32mm] w-[24mm] justify-self-end border border-slate-200 object-cover" height={640} src={value.photo} unoptimized width={480} />}</div>;
  }
  ...
  return <div className={cx("resume-items grid", layout === "cards" ? "grid-cols-2 gap-3" : layout === "compact" ? "gap-2" : "gap-4")}>{(content as ItemsContent).items.map((item) => <article className={cx("resume-item", layout === "cards" && "border border-slate-200 p-3", layout === "standard" && "grid grid-cols-[26mm_1fr] gap-4")} key={item.id}><p className="text-[9px] font-bold text-slate-500">{formatItemPeriod(item)}</p><div><h3 className="text-[11px] font-black">{item.title}</h3><p className="text-[9px] font-bold text-orange-600">{item.subtitle}</p><p className="mt-1 whitespace-pre-line text-[9px] leading-5 text-slate-600">{item.body}</p></div></article>)}</div>;
}
...
function AddSectionDialog({ afterTitle, kind, title, onKind, onTitle, onCancel, onAdd }: { afterTitle: string; kind: SectionKind; title: string; onKind: (kind: SectionKind) => void; onTitle: (title: string) => void; onCancel: () => void; onAdd: () => void }) {
  ...
  return <div className="resume-editor-backdrop fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/60 p-4"><section aria-labelledby="add-resume-section-title" aria-modal="true" className="my-auto w-full max-w-2xl border border-border bg-background shadow-2xl" role="dialog"><header className="flex items-center justify-between border-b border-border p-5"><div><p className="text-[10px] font-bold tracking-widest text-primary">{afterTitle} 다음 위치</p><h2 className="mt-1 text-xl font-extrabold" id="add-resume-section-title">새 섹션 추가</h2></div><button aria-label="추가 창 닫기" className="grid h-10 w-10 place-items-center border border-border" onClick={onCancel}><X className="h-4 w-4" /></button></header><div className="grid gap-5 p-5"><Field label="섹션 제목" value={title} placeholder="예: 오픈소스 활동" onChange={onTitle} /><fieldset><legend className="mb-2 text-xs font-bold text-muted-foreground">내용 형식</legend><div className="grid gap-3 sm:grid-cols-2">{(Object.entries(kinds) as Array<[SectionKind, string]>).map(([value, label]) => <button aria-pressed={kind === value} className={cx("border p-4 text-left", kind === value ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border hover:border-primary/50")} key={value} onClick={() => onKind(value)} type="button"><span className="font-extrabold">{label}</span><span className="mt-3 block border border-slate-200 bg-white p-3">{previews[value]}</span></button>)}</div></fieldset></div><footer className="flex justify-end gap-2 border-t border-border bg-muted/30 p-4"><button className="h-10 border border-border px-4 text-sm font-bold" onClick={onCancel}>취소</button><button className="inline-flex h-10 items-center gap-2 bg-primary px-4 text-sm font-bold text-primary-foreground" onClick={onAdd}><Plus className="h-4 w-4" /> 추가하고 내용 작성</button></footer></section></div>;
}
