...
const alignedModalField = "grid min-w-0 content-start grid-rows-[auto_2.5rem_minmax(2.5rem,auto)] gap-1.5 text-xs font-bold text-muted-foreground";
...
export function ResumeDocumentBuilder() {
  const { state, setState, hydrated, storageStatus, localBackupStatus, lastSavedAt, loadServerCopy, overwriteServerCopy, applyImportCandidate } = useResumeDocumentPersistence();
  ...
  const [aiEditSection, setAiEditSection] = useState<{ id: string; label: string } | null>(null);
  ...
  useEffect(() => {
    if (draft || itemEditor || aiEditOpen || (!insertAfterId && !experienceDialogOpen && !importPanelOpen && !readinessOpen && !sharedSectionDialogOpen && !pdfSnapshot)) return;
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key !== "Escape") return;
      if (pdfSnapshot) setPdfSnapshot(null);
      else if (readinessOpen) setReadinessOpen(false);
      else if (sharedSectionDialogOpen) setSharedSectionDialogOpen(false);
      else if (insertAfterId) setInsertAfterId(null);
      else if (experienceDialogOpen) setExperienceDialogOpen(false);
      else if (importPanelOpen) setImportPanelOpen(false);
    };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [aiEditOpen, draft, experienceDialogOpen, importPanelOpen, insertAfterId, itemEditor, pdfSnapshot, readinessOpen, sharedSectionDialogOpen]);
  ...
  const moveSectionByKeyboard = useCallback((sectionId: string, offset: -1 | 1) => {
    const currentOrder = latestSectionOrderRef.current;
    const nextOrder = moveSectionInOrder(currentOrder, sectionId, offset);
    const title = orderedSections.find((section) => section.id === sectionId)?.title ?? "선택한";
    if (nextOrder === currentOrder) {
      setReorderAnnouncement(`${title} 섹션은 더 이상 ${offset < 0 ? "위" : "아래"}로 이동할 수 없습니다.`);
      return;
    }
    commitSectionOrder(nextOrder);
    setSelectedSectionId(sectionId);
    setReorderAnnouncement(`${title} 섹션을 ${nextOrder.indexOf(sectionId) + 1}번째 위치로 이동했습니다.`);
  }, [commitSectionOrder, orderedSections]);
  ...
  const saveDraft = (nextDraft?: EditDraft) => {
    ...
    setState((current) => {
      ...
      if (savedDraft.scope === "variant" && savedDraft.saveTarget === "parent" && active && activeProfile.customSections.some((section) => section.id === savedDraft.section.id)) return resetSupportVariantSectionToRole(updateRoleCustomSection(current, activeProfile.id, savedDraft.section.id, { title: savedDraft.title.trim() || "새 섹션", content: savedDraft.content }), active.id, savedDraft.section.id);
  ...
  const deleteActiveProfile = () => {
    const dependentVariants = state.variants.filter((variant) => variant.roleProfileId === activeProfile.id);
    const dependentMessage = dependentVariants.length
      ? `\n\n이 직군의 지원처별 버전 ${dependentVariants.length}장도 함께 삭제됩니다:\n${dependentVariants.map((variant) => `- ${variant.name}`).join("\n")}`
      : "";
    if (!window.confirm(`‘${activeProfile.name}’ 직군 이력서를 삭제할까요?${dependentMessage}`)) return;
    setState((current) => deleteRoleProfile(current, activeProfile.id));
  };
  const createShared = () => {
  ...
  const openAiEdit = (section?: Pick<ResumeSection, "id" | "title">) => {
    setAiEditSection(section ? { id: section.id, label: section.title } : null);
    setAiEditOpen(true);
  };
  const closeAiEdit = () => {
    setAiEditOpen(false);
  ...
  return (
    <div className="resume-document-builder wongoji-sharp mx-auto w-full max-w-[1280px] pb-20">
      <header className="resume-builder-chrome border-b-2 border-foreground pb-5">
        <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
          ...
          <div className="resume-builder-actions flex flex-wrap gap-2">
            <button className="inline-flex h-11 items-center gap-2 border border-primary bg-background px-4 text-sm font-bold text-primary disabled:cursor-not-allowed disabled:opacity-50" disabled={!hydrated} onClick={() => openAiEdit()} type="button"><Braces className="h-4 w-4" /> AI로 전체 이력서 작성·수정</button>
            <button className="resume-import-action inline-flex h-11 items-center gap-2 border border-border bg-background px-4 text-sm font-bold text-foreground hover:border-primary hover:text-primary" onClick={() => setImportPanelOpen(true)}><FileUp className="h-4 w-4" /> 자료로 공통 정보 채우기</button><button className="resume-readiness-action inline-flex h-11 items-center gap-2 border border-primary bg-background px-4 text-sm font-bold text-primary" onClick={() => setReadinessOpen(true)}><ClipboardCheck className="h-4 w-4" /> 작성 점검{readinessIssues.length > 0 && <span className="bg-primary px-1.5 py-0.5 text-[10px] text-primary-foreground">{readinessIssues.length}</span>}</button>{view === "resume" && <button className="inline-flex h-11 items-center gap-2 bg-primary px-4 text-sm font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50" disabled={!hydrated} onClick={openPdfPreview}><Printer className="h-4 w-4" /> PDF 미리보기</button>}
          </div>
        </div>
      ...
      {storageStatus === "conflict" && <section aria-live="assertive" className="resume-builder-chrome sticky top-2 z-30 mt-4 border-2 border-red-400 bg-red-50 p-4 text-red-900" role="alert"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="font-extrabold">다른 기기에서 변경된 문서와 충돌했습니다.</p><p className="mt-1 text-xs leading-5">현재 편집은 서버에 저장되지 않습니다. 계속 편집하기 전에 사용할 문서를 선택해 주세요.</p></div><div className="flex shrink-0 flex-wrap gap-2"><button className="h-10 border border-red-300 bg-white px-3 text-xs font-bold" onClick={() => { void loadServerCopy(); }} type="button">서버 문서 불러오기</button><button className="h-10 bg-red-700 px-3 text-xs font-bold text-white" onClick={overwriteServerCopy} type="button">이 편집본으로 저장</button></div></div></section>}
      {localBackupStatus === "error" && <p aria-live="polite" className="resume-builder-chrome mt-3 border border-amber-300 bg-amber-50 px-4 py-3 text-xs font-bold leading-5 text-amber-900">브라우저 임시 저장을 사용할 수 없습니다. 서버 자동 저장은 계속하며, 완료 여부는 서버 저장 상태에서 확인하세요.</p>}

      {view === "shared" ? <SharedManager sections={state.sharedSections} onAdd={() => { prepareNewSection(); setSharedSectionDialogOpen(true); }} onAiEdit={openAiEdit} onDelete={(section) => window.confirm(`‘${section.title}’ 공통 섹션을 모든 직군과 지원 버전에서 삭제할까요?`) && setState((current) => deleteSharedSection(current, section.id))} onEdit={(section) => openEditor("shared", section, section.content)} onLinkExperience={() => setExperienceDialogOpen(true)} onOrder={(sectionOrder) => setState((current) => updateSharedSectionOrder(current, sectionOrder))} /> : (
        <div className="resume-builder-layout mt-6 grid items-start gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <aside className={cx("resume-builder-chrome border border-border bg-card p-5 xl:sticky xl:top-24 xl:block", mobileSettingsOpen ? "block" : "hidden")} id="resume-role-settings">
            ...
            <div className="mt-5 grid gap-4">
              ...
              {state.roleProfiles.length > 1 && <button className="inline-flex h-10 items-center justify-center gap-2 border border-red-200 text-xs font-bold text-red-600" onClick={deleteActiveProfile}><Trash2 className="h-3.5 w-3.5" /> 직군 이력서 삭제</button>}
            </div>
          ...
          <div className="resume-preview-shell min-w-0"><div className="resume-builder-chrome mb-3 flex flex-wrap items-center justify-between gap-2 border border-primary/25 bg-primary/5 px-4 py-3 text-xs"><span className="font-bold text-primary"><span className="hidden md:inline">편집 화면은 A4 너비를 기준으로 표시됩니다.</span><span className="md:hidden">모바일 편집 보기 · PDF는 A4로 저장됩니다.</span></span><span className="font-extrabold">실제 페이지 구분은 PDF 미리보기에서 확인하세요.</span></div><article className="resume-paper mx-auto w-full max-w-[210mm] bg-white text-slate-950 shadow-xl" style={RESUME_DOCUMENT_CSS_VARIABLES}><div className="resume-paper-inner min-h-[297mm] px-[18mm] py-[16mm]"><ResumeEditorHeader company={active?.company ?? ""} role={resolveDocumentRole(activeProfile, active)} /><p className="resume-reorder-help mb-4 flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><GripVertical className="h-3.5 w-3.5" /> 섹션을 선택하면 편집 도구가 표시됩니다. 핸들을 끌거나 포커스 후 위·아래 화살표 키로 순서를 바꿀 수 있습니다.</p><p aria-live="polite" className="sr-only" id="resume-reorder-announcement">{reorderAnnouncement}</p><Reorder.Group axis="y" className="resume-print-sections grid" onReorder={commitSectionOrder} values={orderedSections.map((section) => section.id)}>{orderedSections.map((section) => {
            ...
            const canReset = (!section.custom && ((!active && resolved.source === "role") || (Boolean(active) && resolved.source === "document")))
              || (roleCustom && Boolean(active) && resolved.source === "document");
      ...
      {view === "resume" && <nav aria-label="모바일 이력서 편집" className="resume-mobile-actions">
        ...
        <button onClick={() => setImportPanelOpen(true)} type="button"><FileUp className="h-4 w-4" /> 자료 가져오기</button>
      ...
      {importPanelOpen && <ResumeDocumentImportPanel commonSections={state.sharedSections} sections={orderedSections} workItems={resolvedWorkItems} onApply={applyImportCandidate} onClose={() => setImportPanelOpen(false)} />}
      {aiEditOpen && <ResumeAiJsonEditDialog context={aiEditContext} contextLabel={aiEditContextLabel} sectionId={aiEditSection?.id} sectionLabel={aiEditSection?.label} state={state} onApply={applyAiEdit} onClose={closeAiEdit} onEditAll={() => setAiEditSection(null)} />}
...
function SharedManager({ sections, onAdd, onAiEdit, onDelete, onEdit, onLinkExperience, onOrder }: { sections: ResumeSection[]; onAdd: () => void; onAiEdit: (section: ResumeSection) => void; onDelete: (section: ResumeSection) => void; onEdit: (section: ResumeSection) => void; onLinkExperience: () => void; onOrder: (sectionOrder: string[]) => void }) {
  return <section className="mt-7"><div className="border border-primary/25 bg-primary/5 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-extrabold">공통 정보 · PDF 기본 순서</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">회사, 기간, 성과처럼 직군이 달라도 변하지 않는 사실을 관리합니다. 이 순서는 별도로 조정하지 않은 모든 직군 이력서의 PDF에 그대로 적용됩니다.</p></div><button className="inline-flex h-10 items-center gap-2 bg-primary px-4 text-sm font-bold text-primary-foreground" onClick={onAdd}><Plus className="h-4 w-4" /> 공통 섹션 추가</button></div><div className="mt-4 flex flex-wrap items-center gap-2"><p className="inline-flex items-center gap-2 border border-primary/30 bg-background px-3 py-2 text-xs font-extrabold text-primary"><GripVertical className="h-4 w-4" /> 각 항목의 핸들을 위아래로 끌어 PDF 순서를 변경하세요.</p><button className="inline-flex h-9 items-center gap-2 border border-border bg-background px-3 text-xs font-bold hover:border-primary hover:text-primary" onClick={onLinkExperience} type="button"><FileUp className="h-3.5 w-3.5" /> 확정 경험 검토·가져오기</button></div><p className="mt-2 text-[11px] leading-5 text-muted-foreground">확정 경험은 먼저 변경 내용을 검토하고 선택한 항목만 가져옵니다.</p></div><Reorder.Group axis="y" className="mt-5 grid gap-3" onReorder={onOrder} values={sections.map((section) => section.id)}>{sections.map((section, index) => <SharedSortableCard index={index} key={section.id} onAiEdit={() => onAiEdit(section)} onDelete={isBuiltInResumeSectionId(section.id) ? undefined : () => onDelete(section)} onEdit={() => onEdit(section)} section={section} />)}</Reorder.Group></section>;
}

function SharedSortableCard({ section, index, onAiEdit, onDelete, onEdit }: { section: ResumeSection; index: number; onAiEdit: () => void; onDelete?: () => void; onEdit: () => void }) {
  const dragControls = useDragControls();
  return <Reorder.Item className="border border-border bg-card" dragControls={dragControls} dragListener={false} value={section.id} whileDrag={{ scale: 1.01, zIndex: 20 }}><article className="grid items-center gap-4 p-4 sm:grid-cols-[auto_auto_minmax(0,1fr)_auto]"><button aria-label={`${section.title} 공통 정보 순서 이동`} className="grid h-12 w-12 touch-none cursor-grab place-items-center border border-primary/40 bg-primary/5 text-primary active:cursor-grabbing" onPointerDown={(event) => dragControls.start(event)} type="button"><GripVertical className="h-5 w-5" /></button><span className="grid h-12 w-12 place-items-center bg-foreground text-sm font-black text-background"><span><small className="block text-[8px] font-bold tracking-widest">PDF</small>{String(index + 1).padStart(2, "0")}</span></span><div className="min-w-0"><p className="flex items-center gap-2 text-[10px] font-bold tracking-widest text-primary"><LayoutTemplate className="h-3.5 w-3.5" /> 공통 정보</p><h3 className="mt-1 text-lg font-extrabold" data-common-section-title>{section.title}</h3><p className="mt-1 line-clamp-2 whitespace-pre-line text-sm leading-6 text-muted-foreground">{contentSummary(section) || "아직 작성된 내용이 없습니다."}</p></div><div className="flex flex-wrap gap-2 sm:justify-end"><button className="inline-flex h-10 items-center justify-center gap-2 border border-border bg-background px-3 text-xs font-bold hover:text-primary" onClick={onEdit}><Edit3 className="h-4 w-4" /> 내용 편집</button><details className="relative"><summary aria-label={`${section.title} 공통 섹션 더보기`} className="grid h-10 w-10 cursor-pointer list-none place-items-center border border-border bg-background"><MoreHorizontal className="h-4 w-4" /></summary><div className="absolute right-0 z-40 mt-1 grid w-64 gap-2 border border-border bg-background p-3 shadow-xl"><button className="h-10 border border-primary/40 bg-primary/5 px-3 text-left text-xs font-bold text-primary" onClick={onAiEdit} type="button"><Braces className="mr-1.5 inline h-3.5 w-3.5" /> AI로 이 섹션 작성·수정</button>{onDelete ? <button aria-label={`${section.title} 공통 섹션 삭제`} className="h-10 border border-red-200 bg-background px-3 text-left text-xs font-bold text-red-600" onClick={onDelete} type="button"><Trash2 className="mr-1.5 inline h-3.5 w-3.5" /> 섹션 삭제</button> : <p className="px-2 py-1 text-[11px] font-bold text-muted-foreground">기본 섹션은 삭제할 수 없습니다. 내용은 편집하거나 직군 이력서에서 숨길 수 있습니다.</p>}</div></details></div></article></Reorder.Item>;
}


function SortableSection({ children, onKeyboardMove, onReorderEnd, onSelect, sectionId, selected }: { children: (dragControls: DragControls, onKeyboardMove: (offset: -1 | 1) => void) => React.ReactNode; onKeyboardMove: (offset: -1 | 1) => void; onReorderEnd: () => void; onSelect: () => void; sectionId: string; selected: boolean }) { const dragControls = useDragControls(); return <Reorder.Item aria-label="이력서 섹션" className={cx("resume-print-section resume-editable-section relative", selected && "is-selected")} data-resume-editor-section-id={sectionId} dragControls={dragControls} dragListener={false} layout="position" onClick={onSelect} onDragEnd={onReorderEnd} onFocusCapture={onSelect} role="group" tabIndex={0} transition={{ layout: { duration: .18, ease: "easeOut" } }} value={sectionId} whileDrag={{ opacity: .82, scale: 1.005, zIndex: 20 }}>{children(dragControls, onKeyboardMove)}</Reorder.Item>; }
function DragHandle({ dragControls, onKeyboardMove, title }: { dragControls: DragControls; onKeyboardMove: (offset: -1 | 1) => void; title: string }) { return <button aria-describedby="resume-reorder-announcement" aria-keyshortcuts="ArrowUp ArrowDown" aria-label={`${title} 섹션 순서 이동. 위아래 화살표 키를 사용할 수 있습니다.`} className="resume-drag-handle grid h-10 w-10 shrink-0 touch-none cursor-grab place-items-center border border-slate-300 bg-white text-slate-500 active:cursor-grabbing sm:h-8 sm:w-8" onKeyDown={(event) => { if (event.key === "ArrowUp") { event.preventDefault(); onKeyboardMove(-1); } else if (event.key === "ArrowDown") { event.preventDefault(); onKeyboardMove(1); } }} onPointerDown={(event) => dragControls.start(event)} type="button"><GripVertical className="h-4 w-4" /></button>; }
function HiddenSection({ dragControls, onKeyboardMove, onShow, section }: { dragControls: DragControls; onKeyboardMove: (offset: -1 | 1) => void; onShow: () => void; section: ResumeSection }) { return <div className="resume-hidden flex items-center justify-between gap-2 border border-dashed border-slate-300 bg-slate-50 p-3 text-xs text-slate-500"><span className="flex items-center gap-2 font-bold"><DragHandle dragControls={dragControls} onKeyboardMove={onKeyboardMove} title={section.title} /><EyeOff className="h-4 w-4" /> {section.title} · 숨김</span><button aria-label={`${section.title} 섹션 표시`} className="grid h-9 w-9 place-items-center border border-slate-300 bg-white text-slate-600 hover:border-primary hover:text-primary" onClick={onShow} title="섹션 표시"><Eye className="h-4 w-4" /></button></div>; }

function DocumentSection({ section, content, issueCount, relatedWorkItems, source, onAiEdit, onHide, onReset, resetLabel, onEdit, onItems, onPromote, onDelete, deletePending, dragControls, onKeyboardMove }: { section: ResumeSection; content: SectionContent; issueCount: number; relatedWorkItems: ItemContent[]; source: "shared" | "role" | "document"; onAiEdit: () => void; onHide?: () => void; onReset?: () => void; resetLabel: string; onEdit: () => void; onItems?: () => void; onPromote?: () => void; onDelete?: () => void; deletePending?: boolean; dragControls: DragControls; onKeyboardMove: (offset: -1 | 1) => void }) {
  const sourceLabel = source === "shared" ? "공통 정보" : source === "role" ? "직군 이력서" : "지원 버전 맞춤";
  return <div><div className="resume-section-controls resume-section-toolbar flex items-center justify-between gap-2 border border-dashed border-slate-300 bg-slate-50 p-2"><span className="flex min-w-0 items-center gap-2"><DragHandle dragControls={dragControls} onKeyboardMove={onKeyboardMove} title={section.title} /><span className="min-w-0 truncate text-xs font-extrabold text-slate-700">{section.title}</span><span className="hidden shrink-0 border border-slate-300 bg-white px-2 py-1 text-[10px] font-bold text-slate-500 md:inline-flex">양식 · {sectionTemplateLabel(section)}</span><span className={cx("hidden shrink-0 border px-2 py-1 text-[10px] font-extrabold sm:inline-flex", source === "document" ? "border-orange-300 bg-orange-50 text-orange-700" : source === "role" ? "border-primary/30 bg-primary/5 text-primary" : "border-slate-300 bg-white text-slate-600")}>{section.custom ? "현재 단계 전용" : sourceLabel}</span>{issueCount > 0 && <span className="resume-section-issue-badge shrink-0">확인 {issueCount}</span>}</span><div className="flex shrink-0 items-center gap-1">{onHide && <button aria-label={`${section.title} 섹션 숨기기`} className="grid h-10 w-10 place-items-center border border-slate-300 bg-white text-slate-500 hover:border-primary hover:text-primary sm:h-8 sm:w-8" onClick={onHide} title="섹션 숨기기"><EyeOff className="h-4 w-4" /></button>}<button className="inline-flex h-10 items-center gap-2 border border-slate-400 bg-white px-3 text-xs font-bold hover:border-primary hover:text-primary sm:h-8" onClick={onEdit}><Edit3 className="h-3.5 w-3.5" /> 편집</button><details className="relative"><summary aria-label={`${section.title} 섹션 더보기`} className="grid h-10 w-10 cursor-pointer list-none place-items-center border border-slate-300 bg-white sm:h-8 sm:w-8"><MoreHorizontal className="h-4 w-4" /></summary><div className="absolute right-0 z-40 mt-1 grid w-64 gap-3 border border-slate-300 bg-white p-3 text-slate-700 shadow-xl"><button className="h-9 border border-primary/40 bg-primary/5 px-2 text-left text-xs font-bold text-primary" onClick={onAiEdit} type="button"><Braces className="mr-1 inline h-3.5 w-3.5" /> AI로 이 섹션 작성·수정</button>{onReset && <button className="h-9 border border-primary/40 bg-primary/5 px-2 text-xs font-bold text-primary" onClick={onReset}><RotateCcw className="mr-1 inline h-3.5 w-3.5" /> {resetLabel}</button>}{onItems && <button className="h-9 border border-orange-300 bg-white px-2 text-xs font-bold text-orange-700" onClick={onItems}>{section.id === "experience" ? "경험 선택·편집" : "포함 항목 선택"}</button>}{onPromote && <button className="h-9 border border-primary/40 bg-primary/5 px-2 text-xs font-bold text-primary" onClick={onPromote}><LayoutTemplate className="mr-1 inline h-3.5 w-3.5" /> 공통 섹션으로 전환</button>}{onDelete && <button aria-label={deletePending ? `${section.title} 섹션 삭제 확인` : `${section.title} 섹션 삭제`} className="h-9 border border-red-200 bg-white px-2 text-xs font-bold text-red-600" onClick={onDelete}>{deletePending ? "정말 삭제" : "섹션 삭제"}</button>}</div></details></div></div><ResumeEditorSection currentMonth={currentLocalMonth()} relatedWorkItems={relatedWorkItems} section={{ ...section, content, layout: section.layout ?? "standard" }} /></div>;
}
...
function ItemsEditor({ section, content, workItems, onChange }: { section: ResumeSection; content: ItemsContent; workItems: ItemContent[]; onChange: (content: SectionContent) => void }) {
  ...
  const durationControls = isCareerTimelineSectionId(section.id) && <EditorBlock note={section.id === "experience" ? "PDF 제목 옆에 표시됩니다" : "미리보기와 PDF에 함께 적용됩니다"} title={section.id === "experience" ? "경력 표시 설정" : "경력 상세 표시 설정"}>
    <div className="grid gap-4 sm:grid-cols-2">
      <label className={section.id === "projects" ? alignedModalField : "grid gap-1.5 text-xs font-bold text-muted-foreground"}>시작 연월 정렬<select className="wg-field h-10 px-3 text-sm font-normal" value={content.sortDirection ?? ""} onChange={(event) => updateSortDirection((event.target.value || undefined) as ItemsContent["sortDirection"])}><option value="">수동 순서</option><option value="latest-first">최신순</option><option value="oldest-first">오래된순</option></select>{section.id === "projects" && <span className="text-[11px] font-normal leading-5">수동 순서는 아래 항목에 지정한 순서를 그대로 사용합니다.</span>}</label>
      ...
      {section.id === "projects" && <label className={alignedModalField}>독립 항목 그룹명<input className="wg-field h-10 px-3 text-sm font-normal" maxLength={200} placeholder="독립 프로젝트" value={content.independentGroupTitle ?? ""} onChange={(event) => onChange({ ...content, independentGroupTitle: event.target.value })} /><span className="text-[11px] font-normal leading-5">연결 경력이 없는 항목의 그룹 제목입니다. 비워두면 ‘독립 프로젝트’를 사용합니다.</span></label>}
    </div>
  </EditorBlock>;
...
function CareerDetailControls({ item, workItems, independentGroupTitle, onChange }: { item: ItemContent; workItems: ItemContent[]; independentGroupTitle: string; onChange: (patch: Partial<ItemContent>) => void }) {
  ...
  return <>
    <label className={alignedModalField}>상세 유형<input className="wg-field h-10 min-w-0 w-full appearance-none px-3 text-sm font-normal" maxLength={200} placeholder={detailTypeLabels[item.detailType ?? "project"]} type="text" value={item.detailLabel ?? ""} onChange={(event) => onChange({ detailLabel: event.target.value, itemKind: "career-detail" })} /><span className="text-[11px] font-normal leading-5">자유롭게 입력할 수 있습니다. 비워두면 기본 분류명이 표시됩니다.</span></label>
    <label className={alignedModalField}>연결 경력<select className="wg-field h-10 min-w-0 w-full max-w-full px-3 text-sm font-normal" value={hasValidParent ? item.relatedWorkItemId : unresolved ? "__unresolved" : ""} onChange={(event) => {
    ...
    }}><option value="">{independentGroupTitle}</option>{unresolved && <option value="__unresolved" disabled>연결 확인 필요 · {item.relatedWorkTitle}</option>}{workItems.map((work) => <option key={work.id} value={work.id}>{work.title} · {work.subtitle} · {formatItemPeriod(work)}</option>)}</select><span className={cx("text-[11px] font-normal leading-5", unresolved && "font-bold text-amber-700")}>{unresolved ? "연결 확인 필요" : workItems.length ? "경력을 선택하면 해당 회사 아래에 묶어 표시됩니다." : `연결할 경력이 없어 ‘${independentGroupTitle}’ 그룹에 표시됩니다.`}</span></label>
  </>;
}
...
function ActiveTagGroupEditor({ group, groupTitles, canDelete, onChange, onDelete }: { group: NormalizedTagGroup; groupTitles: string[]; canDelete: boolean; onChange: (group: NormalizedTagGroup) => void; onDelete: () => void }) {
  ...
  const addKeyword = () => {
    const labels = parseTagKeywordDraft(keywordDraft, group.keywords.map((keyword) => keyword.label));
    if (!keywordDraft.trim()) { setKeywordError("추가할 키워드를 입력해 주세요."); return; }
    if (!labels.length) { setKeywordError("모두 이미 이 카테고리에 있는 키워드입니다."); return; }
    updateKeywords([...group.keywords, ...labels.map((label) => ({ id: newTagEditorId("tag-keyword"), label }))]);
    setKeywordDraft("");
    setKeywordError("");
  };
  const saveKeyword = () => {
...
function Editor({ draft, profileCount, profileName, roleVariantCount, variantName, workItems, onChange, onCancel, onSave }: { draft: EditDraft; profileCount: number; profileName: string; roleVariantCount: number; variantName?: string; workItems: ItemContent[]; onChange: (draft: EditDraft) => void; onCancel: () => void; onSave: (draft: EditDraft) => void }) {
  ...
  const choosesTarget = draft.scope === "role" || draft.scope === "variant";
