...
export function ResumeDocumentBuilder() {
  ...
  return (
    <div className="resume-document-builder wongoji-sharp mx-auto w-full max-w-[1280px] pb-20">
      ...
      {draft && <Editor draft={draft} workItems={resolvedWorkItems} onChange={setDraft} onCancel={() => setDraft(null)} onSave={saveDraft} />}
      ...
      {itemEditor && <ItemTailoringDialog profile={activeProfile} variant={itemEditor.scope === "document" ? active : undefined} scope={itemEditor.scope} section={itemEditor.section} workItems={resolvedWorkItems} onState={setState} onClose={() => setItemEditor(null)} />}
...
function SortableSection({ children, onSelect, sectionId, selected }: { children: (dragControls: DragControls) => React.ReactNode; onSelect: () => void; sectionId: string; selected: boolean }) { const dragControls = useDragControls(); return <Reorder.Item aria-label="이력서 섹션" className={cx("resume-print-section resume-editable-section relative", selected && "is-selected")} data-resume-editor-section-id={sectionId} dragControls={dragControls} dragListener={false} onClick={onSelect} onFocusCapture={onSelect} role="group" tabIndex={0} value={sectionId} whileDrag={{ opacity: .8, scale: 1.01, zIndex: 20 }}>{children(dragControls)}</Reorder.Item>; }
...
function StructuredEditor({ section, content, workItems = [], onChange }: { section: ResumeSection; content: SectionContent; workItems?: ItemContent[]; onChange: (content: SectionContent) => void }) {
  ...
  return <div>{durationControls}<div className="grid gap-3">{value.items.map((item, index) => { const endMonthEnabled = isItemEndMonthEnabled(item) && !item.isCurrent; return <fieldset className="border border-border bg-muted/20 p-4" key={item.id}><legend className="px-2 text-xs font-extrabold">항목 {index + 1}</legend><div className="grid gap-3 sm:grid-cols-2"><MonthField label="시작 연월" value={item.startMonth} onChange={(startMonth) => updateItem(item.id, { startMonth })} /><div className="grid gap-2"><MonthField label="종료 연월" value={item.endMonth} disabled={!endMonthEnabled} onChange={(endMonth) => updateItem(item.id, { endMonth, endMonthEnabled: true })} /><label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={endMonthEnabled} type="checkbox" onChange={(event) => updateItem(item.id, { endMonthEnabled: event.target.checked, endMonth: "", ...(isCareerTimelineSectionId(section.id) && event.target.checked ? { isCurrent: false } : {}) })} /> 종료연월 있음</label></div>{isCareerTimelineSectionId(section.id) && <label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={Boolean(item.isCurrent)} type="checkbox" onChange={(event) => updateItem(item.id, { isCurrent: event.target.checked, endMonth: event.target.checked ? "" : item.endMonth, endMonthEnabled: event.target.checked ? false : item.endMonthEnabled })} /> {section.id === "experience" ? "재직 중" : "진행 중"}</label>}<Field label="제목" value={item.title} onChange={(title) => updateItem(item.id, { title })} /><Field label="조직·부제" value={item.subtitle} onChange={(subtitle) => updateItem(item.id, { subtitle })} />{section.id === "projects" && <CareerDetailControls item={item} workItems={workItems} onChange={(patch) => updateItem(item.id, patch)} />}<div className="sm:col-span-2"><TextArea label="설명" value={item.body} onChange={(body) => updateItem(item.id, { body })} /></div></div><button className="mt-3 inline-flex h-9 items-center gap-2 border border-red-200 px-3 text-xs font-bold text-red-600" onClick={() => onChange({ ...value, items: value.items.filter((entry) => entry.id !== item.id) })} type="button"><Trash2 className="h-3.5 w-3.5" /> 이 항목 삭제</button></fieldset>; })}</div><button className="mt-3 inline-flex h-10 items-center gap-2 border border-primary px-4 text-sm font-bold text-primary" onClick={() => onChange({ ...value, items: [...value.items, { id: `item-${Date.now()}`, itemKind: section.id === "experience" ? "work" : section.id === "projects" ? "career-detail" : undefined, detailType: section.id === "projects" ? "project" : undefined, meta: "", startMonth: "", endMonth: "", endMonthEnabled: false, isCurrent: false, title: "", subtitle: "", body: "" }] })} type="button"><Plus className="h-4 w-4" /> 항목 추가</button></div>;
}
...
function CareerDetailControls({ item, workItems, onChange }: { item: ItemContent; workItems: ItemContent[]; onChange: (patch: Partial<ItemContent>) => void }) {
  ...
  return <>
    <label className="grid gap-1.5 text-xs font-bold text-muted-foreground">상세 유형<select className="h-10 border border-border bg-background px-3 text-sm font-normal text-foreground" value={item.detailType ?? "project"} onChange={(event) => onChange({ detailType: event.target.value as ItemContent["detailType"], itemKind: "career-detail" })}>{Object.entries(detailTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    <label className="grid gap-1.5 text-xs font-bold text-muted-foreground">연결 경력<select className="h-10 border border-border bg-background px-3 text-sm font-normal text-foreground" value={hasValidParent ? item.relatedWorkItemId : unresolved ? "__unresolved" : ""} onChange={(event) => {
...
function ItemTailoringDialog({ profile, variant, scope, section, workItems, onState, onClose }: { profile: ResumeRoleProfile; variant?: ResumeDocumentState["variants"][number]; scope: "role" | "document"; section: ResumeSection; workItems: ItemContent[]; onState: React.Dispatch<React.SetStateAction<ResumeDocumentState>>; onClose: () => void }) {
  ...
  return <div className="resume-editor-backdrop fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/60 p-4"><section aria-modal="true" className="my-auto w-full max-w-4xl border border-border bg-background shadow-2xl" role="dialog"><header className="flex items-start justify-between gap-4 border-b border-border p-5"><div><p className="text-[10px] font-bold uppercase tracking-widest text-primary">{scope === "role" ? `${profile.name} 직군 이력서` : `${profile.name} → ${variant?.name}`}</p><h2 className="mt-1 text-xl font-extrabold">{section.title} {section.id === "experience" ? "경험 선택·편집" : "포함 항목 선택"}</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">항목마다 상위 단계 사용, 재작성, 현재 이력서에서 제외를 선택하고 순서를 바꿀 수 있습니다.</p></div><button aria-label="항목 편집 닫기" className="grid h-10 w-10 place-items-center border border-border" onClick={onClose}><X className="h-4 w-4" /></button></header><div className="max-h-[70vh] space-y-3 overflow-y-auto p-5">{ordered.map((item, index) => { const itemSetting = setting?.itemSettings?.[item.id]; const mode = itemSetting?.mode ?? "inherit"; const editable = itemSetting?.content ?? (scope === "document" ? parentById.get(item.id) : item) ?? item; const endMonthEnabled = isItemEndMonthEnabled(editable) && !editable.isCurrent; return <article className="border border-border bg-card p-4" key={item.id}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-extrabold">{item.title || `항목 ${index + 1}`}</p>{item.source && <p className="mt-1 text-[10px] font-bold text-primary">경력기술서 연동 · {item.source.id}</p>}</div><div className="flex gap-1"><button aria-label={`${item.title} 위로`} className="grid h-9 w-9 place-items-center border border-border" disabled={index === 0} onClick={() => move(item.id, -1)}><ArrowUp className="h-3.5 w-3.5" /></button><button aria-label={`${item.title} 아래로`} className="grid h-9 w-9 place-items-center border border-border" disabled={index === ordered.length - 1} onClick={() => move(item.id, 1)}><ArrowDown className="h-3.5 w-3.5" /></button><select aria-label={`${item.title} 항목 방식`} className="h-9 border border-border bg-background px-2 text-xs font-bold" value={mode} onChange={(event) => updateMode(item, event.target.value as ItemMode)}><option value="inherit">{scope === "role" ? "공통 정보 사용" : "직군 이력서 사용"}</option><option value="override">이 단계에서 재작성</option><option value="hidden">{scope === "role" ? "현재 직군 이력서에서 제외" : "현재 지원 이력서에서 제외"}</option></select></div></div>{mode === "override" && <div className="mt-4 grid gap-3 sm:grid-cols-2"><MonthField label="시작 연월" value={editable.startMonth} onChange={(startMonth) => updateContent(item.id, { startMonth })} /><div className="grid gap-2"><MonthField label="종료 연월" value={editable.endMonth} disabled={!endMonthEnabled} onChange={(endMonth) => updateContent(item.id, { endMonth, endMonthEnabled: true })} /><label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={endMonthEnabled} type="checkbox" onChange={(event) => updateContent(item.id, { endMonthEnabled: event.target.checked, endMonth: "", ...(section.id === "experience" && event.target.checked ? { isCurrent: false } : {}) })} /> 종료연월 있음</label></div>{section.id === "experience" && <label className="inline-flex items-center gap-2 text-xs font-bold"><input checked={Boolean(editable.isCurrent)} type="checkbox" onChange={(event) => updateContent(item.id, { isCurrent: event.target.checked, endMonth: event.target.checked ? "" : editable.endMonth, endMonthEnabled: event.target.checked ? false : editable.endMonthEnabled })} /> 재직 중</label>}<Field label="제목" value={editable.title} onChange={(title) => updateContent(item.id, { title })} /><Field label="조직·부제" value={editable.subtitle} onChange={(subtitle) => updateContent(item.id, { subtitle })} />{section.id === "projects" && <CareerDetailControls item={editable} workItems={workItems} onChange={(patch) => updateContent(item.id, patch)} />}<div className="sm:col-span-2"><TextArea label="강조 설명" value={editable.body} onChange={(body) => updateContent(item.id, { body })} /></div></div>}</article>; })}</div><footer className="flex justify-between gap-3 border-t border-border bg-muted/30 p-4"><p className="text-xs text-muted-foreground">변경 사항은 문서에 바로 반영되고 서버에 자동 저장됩니다.</p><button className="h-10 bg-primary px-5 text-sm font-bold text-primary-foreground" onClick={onClose}>완료</button></footer></section></div>;
}
...
function Editor({ draft, workItems, onChange, onCancel, onSave }: { draft: EditDraft; workItems: ItemContent[]; onChange: (draft: EditDraft) => void; onCancel: () => void; onSave: () => void }) {
